...
# factor_data.index = factor_da ta.index.to_timestamp()
...
# # Join the data DataFrame with factor_data
# factor_data = factor_data.join(data['return_1m']).sort_index()

# # Display the first few rows of the factor_data DataFrame
...
def get_clusters(df): 
    df['cluster'] = KMeans(n_clusters=5, random_state=0).fit(df[['return_1m', 'return_2m', 'return_3m', 'return_6m', 'return_9m', 'return_12m']]).labels_
    return df

# data = data.dropna().groupby(level=1, group_keys=False).apply(get_clusters)
